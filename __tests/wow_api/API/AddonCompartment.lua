...
AddonCompartmentMixin = CreateFromMixins(DropdownButtonMixin); -- Not standard, emulates inheritance
...
function AddonCompartmentMixin:RegisterAddons()
	...
	for addonIndex = 1, C_AddOns.GetNumAddOns() do
		...
		if addonEnabled and addonCompartmentFunc and (loadable or reason == "DEMAND_LOADED") then
			...
			addonData.icon = C_AddOns.GetAddOnMetadata(addonIndex, "IconTexture") or
			C_AddOns.GetAddOnMetadata(addonIndex, "IconAtlas");
...
_G.AddonCompartmentFrame = Mixin(CreateFrame("AddonCompartment", "AddonCompartmentFrame", UIParent),
	AddonCompartmentMixin)
AddonCompartmentFrame.Text = AddonCompartmentFrame:CreateFontString()
