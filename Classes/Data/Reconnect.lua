	...
	for name, data in pairs(reconnectData) do
		for session, sessionData in ipairs(lootTable) do
			if sessionData.candidates[name] then -- Should always be there
				sessionData.candidates[name].ilvl = data.ilvl or 0
				...
				sessionData.candidates[name].voters = data.voters and data.voters[session] or {}
				sessionData.candidates[name].votes = data.voters and #sessionData.candidates[name].voters or 0
				sessionData.candidates[name].haveVoted = data.haveVoted and data.haveVoted[session] or false
...
function Reconnect.private:ExtractHaveVoted()
	for name, data in pairs(self.restore) do
		for session, voters in pairs(data.voters or {}) do
