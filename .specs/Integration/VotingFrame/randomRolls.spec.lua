...
describe("#VotingFrame #RandomRolls", function()
	...
	it("full flow", function()
		...
		assert.spy(SetCandidateData).was.called_with(match.is_ref(VotingFrame), 1, "Player1-Realm1", "roll", tonumber(receivedRolls[1]))
		assert.spy(SetCandidateData).was.called_with(match.is_ref(VotingFrame), 1, "Player10-Realm1", "roll", tonumber(receivedRolls[2]))
		assert.spy(SetCandidateData).was.called_with(match.is_ref(VotingFrame), 1, "Player11-Realm1", "roll", tonumber(receivedRolls[3]))
		-- and so on
		assert.spy(SetCandidateData).was.called_with(match.is_ref(VotingFrame), 1, "Player9-Realm1", "roll", tonumber(receivedRolls[20]))
	end)
end)
