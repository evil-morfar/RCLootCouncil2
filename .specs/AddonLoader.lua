   ...
   for x, y in pairs(xmllines) do
      ...
      if y then
         ...
         if ext == ".xml" then
            ...
            Loader.AddTableToTable(Loader.XmlHandler( (level == 0 and basePath or "") .. y, level + 1), ret)
         elseif ext == ".lua" then
            ...
            if string.sub(y, 0, #basePath) ~= basePath then
               y = Loader.NormalizePath(basePath .. y)
            end
            ret[#ret + 1] = y
         end
      end
